...
TOKEN = "34db95dae98b"
MY_NUMBER = "917406890849"  # Insert your number {91}{Your number}
...
def docx_to_markdown(path: str) -> str:
    ...
    return markdownify.markdownify(full_text, heading_style="ATX")

@mcp.tool(description=ResumeToolDescription.model_dump_json())
async def resume() -> str:
    try:
        path = Path("Muthudhanush.docx")
        if not path.exists():
            return "<error>Resume file not found.</error>"
        markdown_resume = docx_to_markdown(str(path))
